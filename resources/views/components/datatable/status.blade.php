...
<div class="form-check form-switch">
    <input class="form-check-input status" style="width:2.3em;height:1.2em" type="checkbox" name="status" @if ($status == 1) checked @endif id="switch-{{$id}}" value="{{$id}}">
    <label class="form-check-label fw-bold ps-2" for="switch-{{$id}}">@if ($status == 1) Active @else Inactive @endif</label>
</div>
@else
@if ($status == 1)
