...
@section('sections')
    <div class="col-12 mt-4 text-end">
        <a href="{{route('admin.comment.trash')}}" class="btn btn-danger mr-3 btn-sm">Trash Comments</a>
    </div>
    <div class="col-12 mt-2">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title m-0">Unapproved Comments</h4>
            </div>
            <div class="card-body">
                <div class="">
...
<script>
    $(document).ready(function() {
        $('#comments').DataTable({
            ...
            "order": [[1, 'asc']],
            deferRender: true,
            ajax: "{{ route('admin.comment.ajax-comments','0') }}",
            columns: [
                {
                    data: 'id',
                ...
                },
                {
                    data: null,
                    render: function(data, type, row) {
                        let trash = "{{route('admin.comment.delete',':id')}}";
                        let del = "{{route('admin.comment.destroy',':id')}}";
                        let approve = "{{route('admin.comment.is_approve',[':id','1'])}}";
                        del = del.replace(':id', data.id);
                        trash = trash.replace(':id', data.id);
                        ...
                        return `<div class="dropdown">
                            ...
                            <ul class="dropdown-menu" style="margin: 0px;">
                                <li><a class="dropdown-item" href="${approve}"><i class="bx bx-check"></i> Approve</a></li>
                                <li><a class="dropdown-item" href="${trash}"><i class="bx bxs-trash"></i> Move to Trash</a></li>
