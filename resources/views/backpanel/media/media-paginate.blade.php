...
@foreach ($media as $item)
    <div class="col-lg-3 col-md-3 col-4 mt-4">
        <div class="file position-relative" data-id="{{$item->id}}" data-dimen="{{$item->dimension}}" data-type="{{$item->type}}" data-size="{{formatBytes($item->size,1)}}" data-path="{{asset('storage/media/'.$item->img)}}" data-alt="{{$item->alt}}" data-name="{{$item->img}}">
