...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function register()
    {
        ...
        View::composer('layouts.frontend.partials.desktop-nav',function($view){
            $loc_id = MenuLocation::where('location','main-menu')->select('id')->first()->id;
            // Main Menu 
            $menuNodes = MenuNodes::with('child')->whereHas('menu',function($query)use($loc_id){
                $query->where('menu_location_id',$loc_id)->where('slug','main-menu');
            })->where('parent_id',0)->select('id','title','url','target','has_child','reference_id','reference_type','route_name')->get();
            // Mega Menu 1
            $megaMenu1 = MenuNodes::with('child')->whereHas('menu',function($query)use($loc_id){
                $query->where('menu_location_id',$loc_id)->where('slug','mega-menu-1');
            })->where('parent_id',0)->select('id','title','url','target','has_child','reference_id','reference_type','route_name')->get();
            // Mega Menu 2
            $megaMenu2 = MenuNodes::with('child')->whereHas('menu',function($query)use($loc_id){
                $query->where('menu_location_id',$loc_id)->where('slug','mega-menu-2');
            })->where('parent_id',0)->select('id','title','url','target','has_child','reference_id','reference_type','route_name')->get();
            // Mega Menu 3
            $megaMenu3 = MenuNodes::with('child')->whereHas('menu',function($query)use($loc_id){
        ...
        View::composer('layouts.frontend.partials.mobile-breaking',function($view){
            $breakingNews = News::where('is_published',1)->where('is_verified',1)->where('status',1)->where('is_featured',1)->inRandomOrder()->select('id','title','slug')->limit(10)->get();
            return $view->with(compact('breakingNews'));
        });
        View::composer('layouts.frontend.partials.desktop-breaking',function($view){
            $breakingNews = News::where('is_published',1)->where('is_verified',1)->where('status',1)->where('is_featured',1)->inRandomOrder()->select('id','title','slug')->first();
            return $view->with(compact('breakingNews'));
        });
        View::composer('layouts.frontend.partials.sidebar-nav',function($view){
            $loc_id = MenuLocation::where('location','sidebar-menu')->select('id')->first()->id;
            $sideMenu = MenuNodes::whereHas('menu',function($query)use($loc_id){
                $query->where('menu_location_id',$loc_id)->where('slug','sidebar');
            })->where('parent_id',0)->select('id','title','url','target','has_child','reference_id','reference_type','route_name','icon')->get();
            return $view->with(compact('sideMenu'));
        });
        View::composer('layouts.frontend.partials.mobile-nav',function($view){
            $loc_id = MenuLocation::where('location','mobile-menu')->select('id')->first()->id;
            $mobileMenu = MenuNodes::whereHas('menu',function($query)use($loc_id){
                $query->where('menu_location_id',$loc_id)->where('slug','mobile-menu');
            })->where('parent_id',0)->select('id','title','url','target','reference_id','reference_type','route_name')->get();
            return $view->with(compact('mobileMenu'));
        });
        View::composer('layouts.frontend.partials.footer',function($view){
            $loc_id = MenuLocation::where('location','footer-menu')->select('id')->first()->id;
